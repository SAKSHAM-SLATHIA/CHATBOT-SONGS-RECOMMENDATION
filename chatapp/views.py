...
def new_thread_view(request):
    ...
    if 'conversation' in request.session:
        ...
        # Initialize a new conversation with just the welcome message
        request.session['conversation'] = []
        welcome_message = {
            'role': 'model', 
            'parts': ["Hi there! I'm Tenor, your music companion. How are you feeling today? I can help suggest songs based on your mood or interests."]
        }
        request.session['conversation'].append(welcome_message)
...
def chat_view(request):
    ...
    if user_input:
        ...
        # Configure the intent checking model
        intent_check_config = {
            "temperature": 0.2,
            "top_p": 0.95,
            "top_k": 64,
        ...
        intent_check_model = genai.GenerativeModel(
            model_name="gemini-2.0-flash",
            generation_config=intent_check_config,
            system_instruction=(
                "Your task is to determine if the user's message is related to music recommendations and contains enough context "
                "for meaningful song suggestions. Consider the conversation history if available. "
                "Return a JSON response with the format: {\"song_related\": boolean, \"has_context\": boolean}. "
                "Both fields should be true only if the message is clearly about music preferences, mood, or song requests "
                "AND contains enough information to make meaningful recommendations."
            ),
        )
        
        # Create message with conversation history for context
        context_messages = []
        # Include up to 5 most recent messages from conversation history
        recent_history = request.session['conversation'][-6:-1] if len(request.session['conversation']) > 5 else request.session['conversation'][:-1]
        
        for message in recent_history:
            context_messages.append(f"{message['role']}: {message['parts'][0]}")
        
        context_message = "\\n".join(context_messages)
        intent_check_message = f"Conversation history:\\n{context_message}\\n\\nCurrent user message: {user_input}"
        
        try:
            # Check intent and context using the intent checking model
            intent_response = intent_check_model.generate_content(intent_check_message)
            intent_data = json.loads(intent_response.text)
            
            is_song_related = intent_data.get('song_related', False)
            has_context = intent_data.get('has_context', False)
            
            if is_song_related and has_context:
                # User message is song-related and has enough context, proceed with song recommendations
                convo = model.start_chat(history=request.session['conversation'][:-1])  # Exclude the latest user message
                ...
                try:
                    ...
                    # If response isn't valid JSON, handle as regular text
                    request.session['conversation'].append({'role': 'model', 'parts': [response.text]})
                    request.session.modified = True
                    music_suggestions = []
                
                # Add model response to conversation history if we got suggestions
                if music_suggestions:
                    # Format response text for chat display
                    formatted_response = "Based on what you've shared, I think you might enjoy these songs:\n\n"
                    for i, song in enumerate(music_suggestions, 1):
                        formatted_response += f"{i}. \"{song.get('Track')}\" by {song.get('Artist')}\n"
                    
                    request.session['conversation'].append({'role': 'model', 'parts': [formatted_response]})
                    request.session.modified = True
                    
                    # Process music suggestions to add to database
                ...
                # User message is not song-related or lacks context, use conversational response
                conversation_config = {
                    "temperature": 0.7,
                    "top_p": 0.95,
                    "top_k": 64,
                ...
                conversation_model = genai.GenerativeModel(
                    ...
                    system_instruction=(
                        "You are Tenor, a friendly music recommendation chatbot. The user's message doesn't contain enough "
                        "information for song recommendations, or isn't related to music. Respond in a friendly way that guides "
                        "the user to share their mood, preferences, or activities so you can suggest appropriate songs. "
                        "Keep your response conversational, concise, and encouraging."
                    ),
                )
                
                convo_response = conversation_model.generate_content(intent_check_message)
                request.session['conversation'].append({'role': 'model', 'parts': [convo_response.text]})
                request.session.modified = True
                
        except Exception as e:
            # Handle any errors during the model interaction
            error_message = "I'm having trouble processing your request right now. Please try again in a moment."
            request.session['conversation'].append({'role': 'model', 'parts': [error_message]})
            request.session.modified = True
...
def song_list_view(request):
    songs = SongRecommendation.objects.filter(user=request.user)
